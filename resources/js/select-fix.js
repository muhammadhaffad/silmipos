...
export default function selectFixFormComponent({
    ...
    return {
        ...
        init: async function () {
            ...
            this.$refs.input.addEventListener('showDropdown', () => {
                const containerSelect = this.select.containerOuter.element;
                const containerSelectList = this.select.dropdown.element;
                const selectBoundingRect = containerSelect.getBoundingClientRect();
                // const selectListBoundingRect = containerSelectList.getBoundingClientRect();
                const td = containerSelect.closest('td');
                td.style.zIndex = '11';
                containerSelectList.style.position = 'fixed'; 
                containerSelectList.style.top = (selectBoundingRect.height + selectBoundingRect.y) + 'px'; 
                containerSelectList.style.width = selectBoundingRect.width + 'px';
                // console.info (selectListBoundingRect , window.innerHeight);
                // console.info(selectBoundingRect, containerSelectList, this.select);
            });
            this.$refs.input.addEventListener('hideDropdown', () => {
                const containerSelect = this.select.containerOuter.element;
                const td = containerSelect.closest('td');
                td.style.zIndex = '10';
            });
