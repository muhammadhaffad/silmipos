...
class PurchaseReturnController extends AdminController
{
    ...
    public function editAllocateRemainingReturnFund($model, $idRetur)
    {
        ...
        $form->tablehasmany('pembelianReturAlokasiKembalianDana', '', function (NestedForm $form) use ($data) {
            ...
            $sisaPembayaran = $row['pembelian_pembayaran']['sisapembayaran'] ?? null;
    ...
    public function detailAllocateRemainingReturnFund($model, $idRetur)
    {
        ...
        })->useTable()->disableCreate()->disableDelete()->value($data->pembelianReturAlokasiKembalianDana->toArray());
        $form->currency('sisakembaliandana', 'Sisa kembalian dana')->setWidth(2, 8)->width('100%')->setGroupClass('mt-4')->symbol('Rp')->readonly()->value($data->kembaliandana);
        $form->html('</div>')->plain();
        $form->disableReset();
        $form->disableCreatingCheck();
    ...
    public function detailReturn(Content $content, $idRetur) 
    {
        ...
        return $content
            ...
            ->description('detail')
            ->row(function (Row $row) use ($idRetur) {
                $row->column(12, $this->detailReturnForm(new PembelianRetur, $idRetur));
                $row->column(12, $this->detailAllocateRemainingReturnFund(new PembelianRetur, $idRetur));
            });
    }
    ...
    {
        try {
            $this->purchaseReturnService->deleteReturn($idRetur);
            admin_toastr('Sukses hapus retur pembelian');
            return [
                'status' => true,
                'then' => ['action' => 'refresh', 'value' => true],
                'message' => 'Sukses hapus retur pembelian'
            ];
        } catch (PurchaseReturnException $e) {
            admin_toastr($e->getMessage(), 'warning');
            return [
    ...
    {
        try {
            $result = $this->purchaseReturnService->updateAllocate($idRetur, $request->all());
