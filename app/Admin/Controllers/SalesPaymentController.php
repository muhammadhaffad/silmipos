...
class SalesPaymentController extends AdminController
{
    protected $salesPaymentService;
    public function __construct(SalesPaymentService $salesPaymentService)
    {
    ...
    {
        $form = new Form($model);
        $form->setAction(route(admin_get_route('sales.payment.store')));
    ...
    public function editPaymentForm($idPembayaran, $model)
    {
        ...
        $form->setAction(route(admin_get_route('sales.payment.update'), ['idPembayaran' => $idPembayaran]));
    ...
    {
        try {
            $result = $this->salesPaymentService->storePayment($request->all());;
            admin_toastr('Sukses buat transaksi pembayaran');
            return redirect()->route(admin_get_route('sales.payment.detail'), ['idPembayaran' => $result->id_penjualanpembayaran]);
    ...
    {
        try {
            $result = $this->salesPaymentService->updatePayment($idPembayaran, $request->all());;
            admin_toastr('Sukses ubah transaksi pembayaran');
            return redirect()->route(admin_get_route('sales.payment.edit'), ['idPembayaran' => $result->id_penjualanpembayaran]);
        } catch (ValidationException $e) {
            return $e->validator->getMessageBag();
        } catch (SalesPaymentException $e) {
            admin_toastr($e->getMessage(), 'warning');
    ...
    {
        try {
            $this->salesPaymentService->deletePayment($idPembayaran);
