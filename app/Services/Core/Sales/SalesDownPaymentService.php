...
class SalesDownPaymentService 
{
    ...
    public function storePayment($request)
    {
        ...
        DB::beginTransaction();
        try {
            $noTransaksi = DB::select("select ('PDP' || lpad(nextval('toko_griyanaura.tr_penjualanpembayaran_dp_seq')::varchar, 6, '0')) as no_transaksi")[0]->no_transaksi;
            $transaksi = Transaksi::create([
                'transaksi_no' => $noTransaksi,
                'id_transaksijenis' => 'penjualanpembayaran_dp',
            ...
            if ($total > $request['totaldp']) {
                throw new SalesPaymentException('Total pembayaran melebihi total DP');
            }
            foreach ($request['penjualanAlokasiPembayaran'] as $item) {
                $newData = [
    ...
    public function updatePayment($idPembayaran, $request)
    {
        ...
        try {
            ...
            $pembayaran->refresh();
            $sisaPembayaran = DB::select('select toko_griyanaura.f_getsisapembayaranpenjualan(?) as sisapembayaran', [$pembayaran->transaksi_no])[0]->sisapembayaran;
            if (0 > $sisaPembayaran) {
                throw new SalesPaymentException('Total pembayaran melebihi sisa pembayaran');
            }
