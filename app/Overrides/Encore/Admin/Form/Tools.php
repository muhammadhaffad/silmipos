...
class Tools implements Renderable
{
     ...
     * @return string
     */
    protected function getListPath($path = null)
    {
        if ($path) {
            return $path;
        }
     ...
     * @return string
     */
    protected function getDeletePath($path = null)
    {
        if ($path) {
            return $path;
        }
     ...
     * @return string
     */
    protected function getEditPath($path = null)
    {
        if ($path) {
            return $path;
        }
        ...
        if ($key) {
            return $this->getListPath().'/edit/'.$key;
        } else {
            return $this->getListPath();
        }
    }
...
<div class="btn-group pull-right" style="margin-right: 5px">
    <a href="{$this->getListPath($path)}" class="btn btn-sm btn-default" title="$text"><i class="fa fa-list"></i><span class="hidden-xs">&nbsp;$text</span></a>
</div>
EOT;
    }
    ...
    public function renderView($path = null, $text = null)
    {
        ...
        return <<<HTML
<div class="btn-group pull-right" style="margin-right: 5px">
    <a href="{$this->getViewPath($path)}" class="btn btn-sm btn-primary" title="{$view}">
